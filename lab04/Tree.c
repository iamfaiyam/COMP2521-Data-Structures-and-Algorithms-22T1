...
static void doTreeSearchBetween(Tree t, Node n, Record lower,
    ...
    int cmp = t->compare(upper, n->rec); 
    int cmp2 = t->compare(lower, n->rec);
    
    // if current record is greater than lower bound go left
    if (cmp2 < 0) {
        doTreeSearchBetween(t, n->left, lower, upper, l); 
    }
    
    // if record is between upper and lower inclusive append record
    if (cmp >= 0 && cmp2 <= 0) {
        ListAppend(l, n->rec);
    }
    
    // if current record is lower than upper bound go right
    if (cmp > 0) {
